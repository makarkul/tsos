...
This page is for the adult(s) running TSOS with a group of learners. It covers
how the project is designed to be taught, a suggested pace, and how to keep a
roomful of mixed-ability learners all productive at once.

TSOS is built around one belief: **people learn TypeScript best by building things
that run, not by being lectured about types.** The whole project is arranged so
that the language shows up exactly when a learner needs it.

---

## The three phases

TSOS is designed to be learned in three phases. Students don't start in the
engine — they build up to it. The kernel is the **destination**, not a no-go zone.

- **Phase 1 — Commands (`src/shell/commands/`).** Each student adds their own
  small command file copied from `_template.ts`. Learns the TypeScript and tooling
  basics, and the edit → save → see-it loop.
- **Phase 2 — Programs (`src/programs/`).** Each student adds their own program
  file that runs as a real process. Learns generators/`yield`, input, loops,
  arrays — and starts watching process states change on the dashboard.
- **Phase 3 — Kernel features (`src/kernel/`).** Now the students go _inside_ the
...
vanish, and a full classroom just works. Phase 3 touches shared engine files,
...
and answering "why is this red?" By Phase 3 it means pairing with students on real
engine changes — you don't have to pre-build the kernel features, you build them
_with_ the students as the payoff of everything they learned in Phases 1 and 2.
The kernel is heavily commented precisely so you can do this together.

---

## The feedback loop is the lesson

Make sure everyone can run `npm run dev` and see the page reload when they save.
That one-second loop — edit, save, see it — does most of the teaching. Protect
it. A student who can _see_ their change is a student who will keep going.
...
| 7–8   | Phase 2 | **Arrays & files** (Rungs 4–5). Students make real little apps.    |
| 9–10  | Phase 2 | **Own types & functions** (Rung 6). Pair stronger students on `rps`, games. |
...
- **Assign files, not features.** "You own `programs/joke.ts`." Ownership removes
  collisions and gives each person something that's clearly _theirs_.
- **A wall of tasks.** Print [FIRST_TASKS.md](FIRST_TASKS.md) or turn each item
  into a sticky note / GitHub issue students can claim.
- **Pair the nervous ones.** Two students per first PR is great for the git steps.
- **Celebrate merges.** When someone's PR merges and their program shows up for
...
- Everyone needs Node.js (LTS) and an editor (VS Code recommended).
- Decide your git flow. The simplest: each person works on their own branch and
  opens a PR; you review and merge. [CONTRIBUTING.md](../CONTRIBUTING.md) §5 walks
  them through it in plain, beginner-friendly language.
...
When the students are ready to go under the hood, good first engine changes (each
is a guided tour of a real OS concept):
...
Have fun. You're not just teaching TypeScript — you're showing students that the
